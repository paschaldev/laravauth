...
class TwoFactorSms implements TokenAuthenticator
{
    ...
    public function tokenMessenger()
    {
        ...
        $message = str_replace('%validity%', intval(config('laravauth.two_factor_sms.lifetime') / 60),
                config('laravauth.two_factor_sms.text_prefix')) . $this->user->{config('laravauth.token_column_name')};
